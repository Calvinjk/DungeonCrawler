...
public class DungeonMapGenerator : MonoBehaviour {
	...
	[Tooltip("Smallest single radius (from door) a room can be")]
	public int minRoomRadius = 2;
	[Tooltip("Largest single dimension (from door) a room can be")]
	public int maxRoomDiameter = 10;
	...
	public Map GenerateMap(int xSize = XDIMDEFAULT, int ySize = YDIMDEFAULT){
		...
		int xLength = Random.Range(minRoomRadius, maxRoomDiameter + 1);
		int yLength = Random.Range (minRoomRadius, maxRoomDiameter + 1);
	...
	void GenerateRoom(int xLocation, int yLocation, int xLength, int yLength, Direction direction){
		if (generationDebugLogs){ 
			...
			"Dimensions of " + xLength + "x" + yLength + "...\n" +
				"Direction: " + direction);
		}
	...
	bool CheckTiles(int xLocation, int yLocation, int xLength, int yLength, Direction direction){
		if (generationDebugLogs) { Debug.Log ("Checking room..."); }
		...
		for (int i = -xLength; i <= xLength; ++i) {
			for (int j = 0; j < yLength; ++j) {
				...
				case Direction.North:
					...
					if (tiles[xLocation + xLength, yLocation + yLength].curTileState != Tile.TileState.Ungenerated){
						// If this tile as already been generated, check if it is a wall
						if (tiles [xLocation + xLength, yLocation + yLength].curTileState != Tile.TileState.Wall) {
				...
				case Direction.South:
					...
					break;
				case Direction.East:
					if (tiles [yLocation + yLength, xLocation + xLength].curTileState != Tile.TileState.Ungenerated) {
						if (tiles [yLocation + yLength, xLocation + xLength].curTileState != Tile.TileState.Wall) {
							if (generationDebugLogs) {
								Debug.Log ("Result: False");
							}	
							return false;
